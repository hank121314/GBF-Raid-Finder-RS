...
use crate::{
  client::redis::Redis,
  common::redis::{gbf_persistence_raid_tweet_key, gbf_raid_boss_key},
  error,
...
enum TweetActorMessage {
  ParseTweet {
    tweet: Tweet,
    respond_to: oneshot::Sender<Option<(RaidBoss, RaidTweet)>>,
  },
  TranslateBossName {
    raid_boss: RaidBoss,
...
impl TweetActor {
  pub fn new(receiver: mpsc::Receiver<TweetActorMessage>, redis: Redis, map: HashMap<String, String>) -> Self {
    TweetActor {
      receiver,
      redis: Arc::new(redis),
  ...
  async fn handle_message(&mut self, msg: TweetActorMessage) -> Result<()> {
    match msg {
      TweetActorMessage::ParseTweet { tweet, respond_to } => match tweet.source.as_str() {
        ...
        GRANBLUE_FANTASY_SOURCE => {
          if let Some((raid_boss, raid_tweet)) = StatusParser::parse(tweet) {
            ...
            let redis_key = gbf_raid_boss_key(raid_boss.borrow(), language);
        ...
        _ => {
          info!("Twitter filter stream find the source which is not from granblue fantasy");
      ...
      TweetActorMessage::PersistRaidTweet { raid_tweet, respond_to } => {
        ...
        let language = Language::from_str(raid_tweet.get_language()).unwrap();
        let redis = self.redis.clone();
        tokio::spawn(async move {
          redis
            .set_protobuf(
              gbf_persistence_raid_tweet_key(raid_tweet.get_boss_name(), language, raid_tweet.get_tweet_id()),
              raid_tweet,
...
impl TweetActorHandle {
  ...
  pub async fn parse_tweet(&self, tweet: Tweet) -> Result<Option<(RaidBoss, RaidTweet)>> {
  ...
  pub async fn translate_boss_name(&self, raid_boss: RaidBoss) -> Result<Option<String>> {
    let (send, recv) = oneshot::channel();
